package kredit.web.util.monthly.model.facade;
...
public class MonthlyFacade {
	...
	private static Logger logger = Logger.getLogger(MonthlyFacade.class);
	public static String environment;
	public static String username;
	public static String password;
	
	//private static Sql2oHelperEOM sql2oEom = new Sql2oHelperEOM(environment, username, password);
	public static List<Saving> getSavingList(ParamMonthly param) {
		...
		try {
			...
			{
				sql += " AND SC.BRANCH_CODE = :KRD_BRANCH ";
			}
		 ...
		 sql += "ORDER BY SC.BRANCH_CODE,sc.cust_ac_no";
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				org.sql2o.Query query = con.createQuery(sql)
							...
							.addParameter("P_REPORT_DT", getEdofMonth(param.getMonth().getId(), param.getMonth().getCode()));
				
				if(!param.getSubBranch().getCode().equals(""))
				{
					query.addParameter("KRD_BRANCH", param.getSubBranch().getCode());
				}
				
				savings = query.executeAndFetch(Saving.class);
			...
			logger.error(
					"Sql2o error while getting Saving List.", e);
		}
		return savings;
	}
	
	public static List<SavingClose> getSavingCloseList(ParamMonthly param) {
		List<SavingClose> savings = new ArrayList<SavingClose>();
		try {
			...
			String sql = "SELECT DISTINCT " +   
				...
				",LTRIM(RTRIM(CF.FIELD_VAL_1))||' '||LTRIM(RTRIM(CF.FIELD_VAL_2)) AS khmer_name " +
				",SC.CCY AS Currency " +
				",NVL(KRD_FUN_GET_EXCH_RATE(1,SC.CCY,:END_DATE),0) AS exch_rate " +
				",IC.MATURITY_DATE AS MATURITY_DATE " +
				",SC.ACCOUNT_CLASS " +
				",AC.DESCRIPTION AS Account_Class_Description " +
				",SC.AC_OPEN_DATE AC_OPEN_DATE " +
				",SC.JOINT_AC_INDICATOR AS Account_Type " +
				",NVL(SC.MODE_OF_OPERATION,'S') AS MODE_OF_OPERATION " +
				",DECODE(SC.CCY,'USD',NVL(TD.TD_AMOUNT,0),ROUND(NVL(TD.TD_AMOUNT,0))) DEPOSIT_AMOUNT " +
				",DECODE(SC.ccy,'USD',nvl(SC.acy_curr_balance,0),ROUND(NVL(SC.ACY_CURR_BALANCE,0))) AS balance_amount " +
				...
				",NVL(ROUND(MONTHS_BETWEEN(IC.MATURITY_DATE, IC.INT_START_DATE)),0) deposit_tenor " +
				",NVL((IC.MATURITY_DATE - IC.INT_START_DATE),0) no_Of_day " +
				",NVL(NVL(KREDIT_FN_TERM_TD_VALUE(IC.ACC,IC.MATURITY_DATE,IC.INT_START_DATE,:END_DATE),KRD_FUN_GET_CASA_INT_RATEDATE(SC.ACCOUNT_CLASS,SC.CCY,:END_DATE)),0) AS interest_rate " +
				",NVL(NVL(KREDIT_FUN_GET_TD_TAX_RATEDATE(SC.CUST_AC_NO,:END_DATE),KRD_FUN_GET_CASA_TAX_RATEDATE(SC.ACCOUNT_CLASS,SC.CCY,:END_DATE)),0) AS tax " +
				",CASE WHEN IC.AUTO_ROLLOVER='Y' AND IC.CLOSE_ON_MATURITY='N' THEN 'AUTO_ROLLOVER' " +
				"WHEN IC.AUTO_ROLLOVER='N' AND IC.CLOSE_ON_MATURITY='Y' THEN 'CLOSE_ON_MATURITY' ELSE '' END auto_rollover_maturity " +
				",CASE WHEN rollover_type='P' AND IC.AUTO_ROLLOVER='Y' THEN 'Principal' " +
				"WHEN rollover_type='S' AND IC.AUTO_ROLLOVER='Y' THEN 'Special Amount' " +
				...
				",SC.AC_STAT_DORMANT " +
				",SC.AC_STAT_FROZEN " +
				",IC.INT_START_DATE INT_START_DATE " +
				",ic.chg_start_date chg_start_date " +
				",NVL(KRD_FUN_GET_TD_ACCR_AMTML(SC.CUST_AC_NO,NVL(:END_DATE,(SELECT D.TODAY  FROM STTM_DATES D WHERE D.BRANCH_CODE= SC.BRANCH_CODE))),KRD_FUN_GET_SAV_ACCR_AMTML(SC.CUST_AC_NO,NVL(:END_DATE,(SELECT D.TODAY  FROM STTM_DATES D WHERE D.BRANCH_CODE= SC.branch_code)))) as Accrual_Int " +
				",NVL(KRD_FUN_GET_TD_TAX_AMTML(SC.CUST_AC_NO,NVL(:END_DATE,(SELECT D.TODAY  FROM STTM_DATES D WHERE D.BRANCH_CODE= SC.BRANCH_CODE))),KRD_FUN_GET_SAV_TAX_AMTML(SC.CUST_AC_NO,:END_DATE)) as Accrual_TAX " +
				",NVL((SELECT IE.ACQUIRED_AMT FROM ICTB_ENTRIES IE WHERE IE.ACC=SC.CUST_AC_NO AND IE.BRN=SC.BRANCH_CODE  AND IE.FRM_NO=1 ),(SELECT distinct IE.ACQUIRED_AMT FROM ICTB_ENTRIES IE WHERE IE.ACC=SC.CUST_AC_NO AND IE.BRN=SC.BRANCH_CODE  AND IE.FRM_NO=1 AND IE.DRCR='C')) AS ACQUIRED_INT_AMT " +
				",nvl((SELECT IE.ACQUIRED_AMT FROM ICTB_ENTRIES IE WHERE IE.ACC=SC.CUST_AC_NO AND IE.BRN=SC.BRANCH_CODE  AND IE.FRM_NO=2 ),(SELECT distinct IE.ACQUIRED_AMT FROM ICTB_ENTRIES IE WHERE IE.ACC=SC.CUST_AC_NO AND IE.BRN=SC.BRANCH_CODE  AND IE.FRM_NO=2 AND IE.DRCR='D')) AS ACQUIRED_TAX_AMT " +
				",IC.BOOK_ACC AS SAVING_ACC " +
				",NVL((SELECT NVL(ITD.OFFSET_ACC, 'O') AS OFFICE FROM ICTM_TDPAYOUT_DETAILS ITD WHERE ITD.ACC = SC.CUST_AC_NO AND ITD.SEQNO=1 GROUP BY ITD.OFFSET_ACC),SC.CUST_AC_NO) DRAWN_DOWN_ACCT " +
				",NVL((SELECT CA1.AC_DESC  FROM ICTM_TDPAYOUT_DETAILS ITD INNER JOIN STTM_CUST_ACCOUNT CA1 ON ITD.ACC = CA1.CUST_AC_NO WHERE CA1.CUST_AC_NO = SC.CUST_AC_NO GROUP BY CA1.AC_DESC),SC.AC_DESC) DRAWN_DOWN_ACCT_NAME " +
				",AP.LAST_LIQ_DT LAST_LIQ_DT " +
				",AP.NEXT_LIQ_DT NEXT_LIQ_DT " +
				",NVL(TD.PROJECTED_INT_TILL_MAT,0)  INT_AM_MATURITY " +
				",NVL(TD.LIQ_AMT,0)  INT_LIQUIDATED_TO_DATE " +
				...
				"LEFT OUTER JOIN ICTM_TD_DETAILS TD ON SC.CUST_AC_NO=TD.ACC AND SC.BRANCH_CODE=TD.BRN AND SC.CCY=TD.CCY " +
				"LEFT OUTER JOIN ICTB_ACC_PR AP ON SC.CUST_AC_NO=AP.ACC AND SC.BRANCH_CODE=AP.BRN " +
				"WHERE TRUNC(SC.checker_dt_stamp) BETWEEN :START_DATE AND :END_DATE " +
				"AND SC.RECORD_STAT='C' " +
				"AND SC.AUTH_STAT='A' ";
		...
		sql +=	"ORDER BY SC.BRANCH_CODE";
			
			try (Connection con = Sql2oHelper.sql2o.open()) {
				org.sql2o.Query query = con.createQuery(sql)
						.addParameter("USER_NAME", UserCredentialManager.getIntance().getLoginUsr().getUsername())
						.addParameter("START_DATE", getSdofMonth(param.getMonth().getId(), param.getMonth().getCode()))
						.addParameter("END_DATE", getEdofMonth(param.getMonth().getId(), param.getMonth().getCode()));
				...
				savings = query.executeAndFetch(SavingClose.class);
				
				if (savings == null) {
					savings = new ArrayList<SavingClose>();
				}
			}
			
		} catch (Exception e) {
			logger.error(
					"Sql2o error while getting Saving Close List.", e);
		}
		return savings;
	}
	
	public static List<Loan> getLoanList(ParamMonthly param) {
		...
		try {
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				...
				loans = query.executeAndFetch(Loan.class);
			}
			...
			logger.error(
					"Sql2o error while getting Loan List.", e);
		}
		return loans;
	}
	
	public static List<LoanClose> getLoanCloseList(ParamMonthly param) {
		List<LoanClose> loans = new ArrayList<LoanClose>();
		try {
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				...
				loans = query.executeAndFetch(LoanClose.class);
			}
			
			if (loans == null) {
				loans = new ArrayList<LoanClose>();
			}
			
		} catch (Exception e) {
			logger.error(
					"Sql2o error while getting Loan Close List.", e);
		}
		return loans;
	}
	
	public static List<LoanDrop> getLoanDropList(ParamMonthly param) {
		List<LoanDrop> loans = new ArrayList<LoanDrop>();
		try {
			...
			String sql = "SELECT CL.ACCOUNT_NUMBER As account_no " +
				...
				"WHERE  m.ORIGINAL_ST_DATE <= :END_DATE AND " +
				"KREDIT_FUN_LOAN_OUTSTANING(m.ACCOUNT_NUMBER,:END_DATE) >0 " +
				")";
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				...
				loans = query.executeAndFetch(LoanDrop.class);
			}
			
			if (loans == null) {
				loans = new ArrayList<LoanDrop>();
			}
			
		} catch (Exception e) {
			logger.error(
					"Sql2o error while getting Loan Drop List.", e);
		}
		return loans;
	}
	
	public static List<LoanVsu> getLoanVSUList(ParamMonthly param) {
		List<LoanVsu> loans = new ArrayList<LoanVsu>();
		try {
		...
		sql +=  "ORDER BY CL.BRANCH_CODE,CL.ACCOUNT_NUMBER";
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				...
				loans = query.executeAndFetch(LoanVsu.class);
			}
			
			if (loans == null) {
				loans = new ArrayList<LoanVsu>();
			}
			
		} catch (Exception e) {
			logger.error(
					"Sql2o error while getting LoanVSU List.", e);
		}
		return loans;
	}

	public static List<LoanWriteOff> getLoanWriteOffList(ParamMonthly param) {
		List<LoanWriteOff> loans = new ArrayList<LoanWriteOff>();
		try {
			...
			String sql = "SELECT CL.ACCOUNT_NUMBER As account_no " +
				...
				"LEFT OUTER JOIN STTM_CUST_PERSONAL CODP ON CODP.CUSTOMER_NO = COD.CUSTOMER_NO " + 
				"WHERE CL.ACCOUNT_STATUS='L' " +
				"AND TO_DATE(to_char(KREDIT_CLPKS_LOAN.FN_LAST_PAYMENT_DATE(CL.ACCOUNT_NUMBER,CL.BRANCH_CODE,:END_DATE),'DD-MON-yyyy'),'DD-MON-yyyy')<=  :END_DATE " +
				"AND KREDIT_fun_loan_outstaning(CL.ACCOUNT_NUMBER,:END_DATE) = 0 " +
				"AND CL.ACCOUNT_NUMBER IN(SELECT DISTINCT EV.ACCOUNT_NUMBER FROM CLTB_EVENT_ENTRIES EV WHERE EV.DR_ACC='671109111' AND EV.COMPONENT_NAME<>'PROV')"; 
			...
			try (Connection con = Sql2oHelper.sql2o.open()) {
				...
				loans = query.executeAndFetch(LoanWriteOff.class);
			}
			
			if (loans == null) {
				loans = new ArrayList<LoanWriteOff>();
			}
			
		} catch (Exception e) {
			logger.error(
					"Sql2o error while getting Loan WriteOff List.", e);
		}
		return loans;
	}
	...
	public static Date getSdofMonth(int num, String year)
	{
		Calendar calendar = Calendar.getInstance();
		calendar.set(Integer.parseInt(year), num, 1);
    
		return calendar.getTime();
	}
	
	public static Date getEdofMonth(int num, String year)
	{
		Calendar calendar = Calendar.getInstance();
	...
	public static int removeOldIncData(Date dat, String table) {
		int result = 0;
		try {
			QueryRunner queryRunner = new QueryRunner();
			java.sql.Connection conn = OracleHelper.getConnection();
			String sql = "DELETE FROM " + table + " WHERE TRUNC(REPORT_DATE) = ?";
			queryRunner.update(conn, sql, getOracleformatDate(dat));
			DbUtils.close(conn);
		} catch (Exception e) {
			logger.error("DBUtils error while removing Incentive Old Data.", e);
		}
		return result;
	}
	
	public static String getOracleformatDate(Date date)
	{
		String result = new SimpleDateFormat("dd-MMM-yy").format(date);
