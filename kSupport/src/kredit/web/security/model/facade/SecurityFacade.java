...
public class SecurityFacade {
	...
	public static List<CodeItem> getBranchesList() {
		...
		try {
			String sql = "SELECT id, branch_code code, name_en description FROM SYS_BRANCH ORDER BY branch_code";
	...
	public static List<User> getSyncUserList() {
		...
		try {
			...
			String sql = "SELECT u.EXT_USER_REF EMP_ID, b.ID BRANCH_ID, u.HOME_BRANCH BRANCH_CODE, "
					...
					+ "WHERE USER_ID NOT IN (SELECT SYS_USER.USERNAME FROM SYS_USER) AND u.RECORD_STAT = 'O'";
			*/
			
			String sql = "SELECT u.EXT_USER_REF EMP_ID, b.ID BRANCH_ID, u.HOME_BRANCH BRANCH_CODE, " +
						"b.NAME_EN BRANCH_NAME, u.USER_NAME FULL_NAME, LOWER(e.fname_en) || '_' || LOWER(e.lname_en) USERNAME, " +
						"ENC_PWD(TO_CHAR(e.birth_date, 'ddmmyy')) PWD, 0 PWD_CHANGE_ON, 'N' PWD_CHANGE_FORCE, null TIME_LEVEL, " +
						"null AUTHORIZED, null START_ON, null END_ON, 0 SUCCESS, 0 FAIL, 'N' LOCKED, 'A' STATUS, :name CREATE_BY, " +
						":date CREATE_ON, null CHANGE_BY, null CHANGE_ON, 0 SECURITY_NO " +
						"FROM SMTB_USER u INNER JOIN SYS_BRANCH b ON u.HOME_BRANCH = b.BRANCH_CODE AND b.ID <> 342 " +
						"LEFT JOIN SYS_EMPLOYEE e ON u.ext_user_ref = e.emp_id " +
						"WHERE LOWER(e.fname_en) || '_' || LOWER(e.lname_en) NOT IN (SELECT us.USERNAME FROM SYS_USER us WHERE us.STATUS = 'A' AND us.USERNAME IS NOT NULL) AND u.RECORD_STAT = 'O' " +
						"AND e.BIRTH_DATE IS NOT NULL " +
						//IGNORED USER_NAME
						"AND u.USER_NAME NOT IN ('KOEUT, Vanna ', 'MIGRATION2', 'NETH, Sovathena', 'EL, Srey Touch', 'SAMANG, Den')";
	...
	public static void updateUserStatus() {
		...
		try {
			String sql = "update ( " +
						 "	SELECT u.status old_stat " +
						 "	FROM SYS_USER u INNER JOIN SMTB_USER s on u.username = s.user_id " +
						 "	WHERE s.record_stat = 'C' AND u.status = 'A' " +
						 ") t set t.old_stat = 'I'";
