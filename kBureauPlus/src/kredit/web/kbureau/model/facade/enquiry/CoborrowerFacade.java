...
public class CoborrowerFacade {
	private static Logger logger = Logger.getLogger(CoborrowerFacade.class);
	
	public static Enquiry getCoborrower(String acc){
		...
		try{
			String sql = "SELECT 'NA' AS enquiryType"
					...
					+ " WHERE CC.type ='C' and cc.validated='Y' and CC.ACCOUNT_NUMBER = '"+acc+"'";
	...
	public static List<Enquiry> getCoborrowerList(ParamEnquiry param){
		...
		try{
			String sql = ""
					...
					+ "INNER JOIN SYS_BRANCH BR ON SB.PARENT_ID = BR.ID where 1=1 and cg.type ='C' and validated='Y' "			
					+ "  and CL.CURRENCY = NVL(:currency, CL.CURRENCY)"	
					+ " AND CL.ACCOUNT_STATUS <> 'V'  "
					+ " AND ((CL.ACCOUNT_NUMBER NOT IN(SELECT account_no FROM cbc_enquiry WHERE status = 'O' and type='C')) OR "
		...
		try(org.sql2o.Connection con = Sql2oHelper.sql2o.open()) {
			...
			if(enquiry.getStatus().equals("OK")){
				...
				if(checkRef(enquiry.getRefNumber()) > 0){
					...
					sql = "INSERT INTO CBC_ENQUIRY(ID, REF_NO, ACCOUNT_NO, STATUS,TYPE) VALUES(CBC_ENQUIRY_SEQ.NEXTVAL,:ref,:acc,:status,'C')";
					con.createQuery(sql)
						.addParameter("ref", enquiry.getRefNumber())
